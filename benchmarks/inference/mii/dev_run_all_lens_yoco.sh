...
OUT_DIR=./DEV_results_yocov2_sans_DA
...
for MODEL in ${MODELS[@]}; do
    if [[ $MODEL == *"yoco"* ]]; then
        ...
        BACKEND="yoco"
        echo "[bash] backend=$BACKEND"
    fi
    echo "*** BACKEND $BACKEND LOAD_FORMAT $LOAD_FORMAT***"
    TOKENIZERS_PARALLELISM=true python ./run_benchmark.py --backend ${BACKEND}  --model ${MODEL} --mean_prompt_length 500 --mean_max_new_tokens 500 --tp_size 1 --out_json_dir ${OUT_DIR} --load_format ${LOAD_FORMAT} --stream --overwrite_results --warmup 0 --num_requests 32
