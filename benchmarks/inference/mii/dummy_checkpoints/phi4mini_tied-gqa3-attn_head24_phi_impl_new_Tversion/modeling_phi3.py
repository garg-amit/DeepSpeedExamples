...
class Phi3MLP(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.gate_up_proj = nn.Linear(config.hidden_size, 2 * config.intermediate_size, bias=True)
...
class Phi3Attention(nn.Module):
    ...
    def __init__(self, config: Phi3Config, layer_idx: Optional[int] = None):
        ...
        self.qkv_proj = nn.Linear(self.hidden_size, op_size, bias=True)
        self._init_rope()

    def _init_rope(self):
        if self.rope_scaling is None:
            if self.config._attn_implementation == "flash_attention_2" and is_flash_attn_2_available():
