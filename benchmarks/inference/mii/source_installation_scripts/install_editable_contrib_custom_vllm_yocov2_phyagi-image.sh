#!/bin/bash
# source me!
conda create -n myenveditablecontribphyagi python=3.10 -y
## If conda activate fails, then need to run this command 
source activate myenveditablecontribphyagi
conda activate myenveditablecontribphyagi
...
cd /data/users/adatkins/dev/phivnext/yoco/yocov2/contribute_ccrb_vllm
