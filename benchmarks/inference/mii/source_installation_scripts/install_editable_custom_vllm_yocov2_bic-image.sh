...
conda create -n myenveditable python=3.10 -y
# If conda activate fails, then need to run this command 
source activate myenveditable
conda activate myenveditable
# verify that the correct env is used by
conda env list

cd /data/users/adatkins/dev/phivnext/yoco/yocov2/modded_vllm
